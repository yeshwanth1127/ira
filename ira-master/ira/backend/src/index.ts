...
app.listen(port, () => {
  ...
  if (!config.internalApiKey) {
    console.warn(
      "[ira] INTERNAL_API_KEY is empty — license minting (POST /licenses/internal/issue) will return 403. " +
  ...
  if (dbg) {
    console.log(
      `[ira-debug] INTERNAL_API_KEY ${config.internalApiKey ? `set (${config.internalApiKey.length} chars)` : "NOT SET"}`,
    );
  }
});
