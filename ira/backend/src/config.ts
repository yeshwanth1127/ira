 ...
 */
export function customerJwtVerificationSecrets(): string[] {
  const parts = [
...
export function assertConfig() {
  if (!config.jwtAccessSecret) throw new Error("JWT_ACCESS_SECRET is missing");
  if (!config.licenseKeySecret) throw new Error("LICENSE_KEY_SECRET is missing");
}
