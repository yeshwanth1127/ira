...
router.post("/register", asyncHandler(async (req, res) => {
  ...
  try {
    ...
    let trial_license_key: string | null = null;
    let trial_license_id: string | null = null;
    if (plan) {
...
router.post("/exchange-customer-jwt", asyncHandler(async (req, res) => {
  ...
  let userId: string;
  try {
    const decoded = jwt.verify(parsed.data.customer_token, config.customerJwtSecret) as { sub?: string };
    if (typeof decoded.sub !== "string" || !decoded.sub) throw new Error("bad sub");
    userId = decoded.sub;
  } catch {
...
router.post("/refresh", asyncHandler(async (req, res) => {
  ...
  if (sess.revoked_at) return res.status(401).json({ error: "Refresh token revoked" });
  if (new Date(sess.expires_at).getTime() < Date.now()) return res.status(401).json({ error: "Refresh token expired" });
