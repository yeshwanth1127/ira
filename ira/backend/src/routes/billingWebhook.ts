...
router.post("/", async (req, res) => {
  ...
  if (!config.razorpayWebhookSecret) return res.status(500).json({ error: "Webhook not configured" });
  ...
  try {
    await dbQuery(
      "INSERT INTO webhook_events(provider, event_id, raw) VALUES ('razorpay',$1,$2::jsonb)",
      [String(providerEventId), JSON.stringify(body)],
    );
  } catch (e: any) {
  ...
  try {
    if (event.startsWith("subscription.")) {
      ...
      if (sub?.id) {
        ...
        if (status === "active") {
          const internalSub = await dbQuery<{ id: string; user_id: string; plan_id: string; current_period_end: Date }>(
            "SELECT id, user_id, plan_id, current_period_end FROM subscriptions WHERE provider_subscription_id=$1",
            [sub.id],
          );
    ...
    if (event.startsWith("payment.")) {
      ...
      if (pay?.id) {
        ...
        if (subId) {
          const internalSub = await dbQuery<{ id: string }>("SELECT id FROM subscriptions WHERE provider_subscription_id=$1", [
            String(subId),
          ]);
        ...
        if (internalSubId) {
          const u = await dbQuery<{ user_id: string }>("SELECT user_id FROM subscriptions WHERE id=$1", [internalSubId]);
          userId = u.rows[0]?.user_id ?? null;
        }
        if (!userId) userId = body?.payload?.payment?.entity?.notes?.user_id ?? null;
