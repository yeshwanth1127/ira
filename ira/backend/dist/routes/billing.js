...
router.post("/razorpay/checkout", requireUser, async (req, res) => {
    ...
    if (!plan)
        return res.status(404).json({ error: "Plan not found or not configured for Razorpay" });
    ...
    const custRes = await dbQuery("SELECT razorpay_customer_id FROM razorpay_customers WHERE user_id=$1", [userId]);
    let razorpayCustomerId = custRes.rows[0]?.razorpay_customer_id;
    if (!razorpayCustomerId) {
        const customer = await rp.customers.create({ email, fail_existing: "0" });
