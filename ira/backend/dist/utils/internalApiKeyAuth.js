...
export function internalApiKeyDebugEnabled() {
    const v = process.env.DEBUG_IRA_INTERNAL || process.env.DEBUG_IRA;
    return v === "1" || v?.toLowerCase() === "true";
}
export function readInternalApiKeyHeader(req) {
    const h = req.headers["x-internal-api-key"];
    if (typeof h === "string")
        return h.trim();
    if (Array.isArray(h) && h[0] !== undefined)
        return String(h[0]).trim();
    return undefined;
}
export function internalApiKeyMatches(req, configured) {
 ...
 */
export function logInternalApiKeyForbidden(routeLabel, req, configured) {
    const key = configured.trim();
    const header = readInternalApiKeyHeader(req);
    const rid = req.requestId;
    const parts = [];
    if (!key)
        parts.push("INTERNAL_API_KEY is empty in IRA backend (.env)");
    if (header === undefined || header === "")
        parts.push("no x-internal-api-key header (or empty after trim)");
    ...
    if (internalApiKeyDebugEnabled()) {
        ...
        const origLen = typeof orig === "string" ? orig.length : Array.isArray(orig) ? orig.join(",").length : 0;
        console.warn(`[ira-debug] ${req.method} ${req.originalUrl ?? req.url} content-type=${req.headers["content-type"] ?? "(none)"} ` +
            `x-internal-api-key raw bytes≈${origLen} trimmed header len=${header?.length ?? 0} env key len=${key.length}`);
    }
}
//# sourceMappingURL=internalApiKeyAuth.js.map